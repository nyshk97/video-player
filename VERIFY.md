...
- 「フルアクセスを許可」後、5列グリッドで動画が表示される (古い → 新しい、最新が下)
- 一覧右上の並び順メニューで「撮影日」を選ぶと、撮影日が新しい動画ほど下に並ぶこと
- iOS 26 以降では、一覧右上の並び順メニューに「ライブラリ追加日」が表示されること
- iOS 26 以降で「ライブラリ追加日」を選ぶと、ライブラリに最近追加した動画ほど下に並ぶこと
- iOS 26 未満では、一覧右上の並び順メニューに「ライブラリ追加日」が表示されないこと
- アプリを再起動しても、選択中の並び順が維持されること
...
- 動画を複数本入れた状態で左スワイプ → 現在の並び順で次の動画 (より新しい動画) に切り替わること
- 右スワイプ → 現在の並び順で前の動画 (より古い動画) に切り替わること
- 左右スワイプ中、動画画面が指の移動に追従して横へ動くこと
- 閾値未満で指を離すと、動画画面が元の位置へ自然に戻ること
- 閾値を超えて指を離すと、現在動画が横へ抜けて、切り替え先動画が反対側から入ること
- 一時停止中に左右スワイプした場合、切り替え先も一時停止状態で先頭に表示されること
- 先頭・末尾で範囲外方向にスワイプした場合、動画が切り替わらずプレイヤーが閉じないこと
- 左右スワイプ後もシングルタップ overlay、左右ダブルタップ seek、左右長押し、下スワイプ dismiss が動くこと
- 左右スワイプ直後のローディング中は、タップ・ダブルタップ・長押し・下スワイプ dismiss・編集/削除操作が発火しないこと
...
- 1.5x を選択 → 動画画面の右半分を長押し → 押している間だけ 2x で再生 → 離すと 1.5x に戻る
- 再生中に動画画面の左半分を長押し → 押している間だけ約 2x の速さで巻き戻る → 離すと元の再生速度で再開する
